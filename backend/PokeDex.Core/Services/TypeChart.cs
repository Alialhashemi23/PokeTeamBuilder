...
{
    /// <summary>
    /// The standard 18-type effectiveness chart (Gen 6+).
    /// Multipliers are attacking type vs defending type; anything not listed is 1x.
    /// </summary>
    public static class TypeChart
    {
        ...
        private static readonly Dictionary<(string Attacking, string Defending), double> Overrides =
            new()
            {
                ...
                [("Ghost", "Normal")] = 0, [("Ghost", "Psychic")] = 2, [("Ghost", "Ghost")] = 2,
                [("Ghost", "Dark")] = 0.5,
        ...
        /// <summary>
        /// Multiplier for an attacking type hitting a single defending type. 1.0 when neutral.
        /// </summary>
        public static double GetMultiplier(string attacking, string defending)
        {
            var key = (Normalize(attacking), Normalize(defending));
            return Overrides.TryGetValue(key, out var multiplier) ? multiplier : 1.0;
        }

        /// <summary>
        /// Combined defensive multiplier for a (possibly dual-typed) Pokemon.
        /// </summary>
        public static double GetDefensiveMultiplier(string attacking, string primary, string? secondary)
        {
            var multiplier = GetMultiplier(attacking, primary);
            if (!string.IsNullOrEmpty(secondary))
            {
                multiplier *= GetMultiplier(attacking, secondary);
            }
            return multiplier;
        }

        private static string Normalize(string type) =>
            string.IsNullOrEmpty(type) ? type : char.ToUpperInvariant(type[0]) + type[1..].ToLowerInvariant();
    }
}
